...
echo "Number of shape candidates, less is faster but has lower accuracy, default is 500 : "
read numCandidate

echo "Number of shape mutates, less is faster but has lower accuracy, default is 100 : "
read numMutates

for i in *.$inExt ; do echo $i ; geometrize_g++ -i $i -o g-$i."$outExt" -s "$numShapes" -t "$setMod" -c "$numCandidate" -m "$numMutates" ; done 
